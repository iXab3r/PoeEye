...
public abstract class BlazorWindowMouseDragControllerBase : DisposableReactiveObject
{
        ...
        if (draggingStarted && lastMouseMoveAt != DateTimeOffset.MinValue)
        {
            ...
            if (gap >= DragGapWarningThreshold)
            {
                var cursorDelta = hasLastCursorPosition
                    ...
                    : Size.Empty;
                BlazorWindow.Log.Warn(
                    $"Drag move dispatch gap detected: GapMs={gap.TotalMilliseconds:F0}, MoveCount={moveIndex}, Cursor={current}, CursorDelta={cursorDelta}, WindowRect={BlazorWindow.GetWindowRect()}, MouseCaptured={ContentControl.IsMouseCaptured}");
            }
        }
    ...
    private void ControlOnMouseUp(object sender, MouseButtonEventArgs e)
    {
        BlazorWindow.Log.Debug(
            $"MouseUp observed by drag controller. Disposing drag session. Cursor={GetCursorPosition()}, WindowRect={BlazorWindow.GetWindowRect()}, MouseCaptured={ContentControl.IsMouseCaptured}");
        Dispose();
    }
