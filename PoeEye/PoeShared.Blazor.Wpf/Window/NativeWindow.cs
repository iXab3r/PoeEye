...
/// <summary>
/// Blazor-agnostic core of the thread-safe windowing model: dedicated dispatcher, single command channel
/// for all changes, timestamped property mirror (last-wins), lazily-created inner WPF window.
/// Hosts arbitrary WPF content supplied via <see cref="ContentFactory"/>.
/// <see cref="BlazorWindow"/> specializes this class with Blazor/WebView2 content.
/// Things to note:
...
internal partial class NativeWindow : DisposableReactiveObjectWithLogger, INativeWindow, IBlazorWindowMetroController, IBlazorWindowAutomationIdentity
{
    ...
    private readonly Lazy<WindowView> windowSupplier;
    ...
    /// <para>
    /// Threading contract: the factory is ALWAYS invoked on the window's own UI (dispatcher) thread,
    /// never on the caller thread. It is invoked lazily - the first time when the inner WPF window is
    /// created (on first Show/ShowDialog), and again every time the property is re-assigned afterwards.
    /// Re-assignment after the window is shown is supported: the new factory is re-invoked on the UI
    /// thread and the produced element replaces the previous content (updates are serialized through
    /// the window command queue, last-wins). Setting the property to null clears the hosted content.
    /// </para>
    /// <para>
    /// The produced element must be created INSIDE the factory - WPF elements have thread affinity and
    /// cannot be created upfront on the caller thread.
    /// </para>
    /// </summary>
    public virtual Func<INativeWindow, UIElement> ContentFactory { get; set; }
    ...
    public IDisposable EnableDragMove()
    {
        ...
        // Keep drag start serialized with the rest of the window command queue.
    ...
    public IDisposable EnableResize(WindowResizeDirection direction)
    {
        ...
        // Keep resize start serialized with the rest of the window command queue.
        var anchor = new CompositeDisposable();
    ...
    private protected void EnqueueUpdate(IWindowEvent windowEvent)
    {
    ...
    private WindowView GetOrCreate()
    {
        WindowView window;
        if (!windowSupplier.IsValueCreated)
        {
            ...
            windowSubscriptionAnchor.Disposable = SubscribeToWindow(Log, window)
                .SubscribeSafe(x =>
                {
                    EnqueueUpdate(x);
                }, Log.HandleUiException);
            Log.Debug("Window view created and subscribed successfully");
        }
        else
        {
            window = windowSupplier.Value;
    ...
    /// <summary>
    /// Creates the inner WPF window view. Invoked lazily on the window's own UI thread.
    /// Subclasses provide their own view (e.g. Blazor-hosting view).
    /// </summary>
    private protected virtual WindowView CreateWindowView()
    {
