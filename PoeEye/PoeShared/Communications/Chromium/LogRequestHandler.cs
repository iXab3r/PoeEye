...
namespace PoeShared.Communications.Chromium
{
    internal sealed class LogRequestHandler : IRequestHandler
    {
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnBeforeBrowse executed, request: {request.Url}");
            return false;
        }
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnCertificateError executed");
            return false;
        }

        public void OnPluginCrashed(IWebBrowser browserControl, IBrowser browser, string pluginPath)
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnPluginCrashed executed, pluginPath: {pluginPath}");
        }

        public CefReturnValue OnBeforeResourceLoad(IWebBrowser browserControl, IBrowser browser, IFrame frame, IRequest request, IRequestCallback callback)
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnBeforeResourceLoad executed, request: {request.Method} '{request.Url}'");
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] GetAuthCredentials executed");
            //Return true to continue the request and call CefAuthCallback::Continue() when the authentication information is available. Return false to cancel the request.
            return false;
        }
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnSelectClientCertificate executed");
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnRenderProcessTerminated executed, status: {status}");
        }

        public bool OnQuotaRequest(IWebBrowser browserControl, IBrowser browser, string originUrl, long newSize, IRequestCallback callback)
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnQuotaRequest executed, originUrl: {originUrl}, newSize: {newSize}");
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnResourceRedirect executed, request: {request.Url}, newUrl: {newUrl}");
        }

        public bool OnProtocolExecution(IWebBrowser browserControl, IBrowser browser, string url)
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnProtocolExecution executed, url: {url}");
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnRenderViewReady executed");
        }

        public bool OnResourceResponse(IWebBrowser browserControl, IBrowser browser, IFrame frame, IRequest request, IResponse response)
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnResourceResponse executed, request: {request.Method} '{request.Url}'");
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] GetResourceResponseFilter executed");
        ...
        {
            Log.Trace($"[Chromium{GetBrowserId(browserControl)}] OnResourceLoadComplete executed, {request.Method} '{request.Url}'");
        }
