namespace PoePickitTestApp
{
    internal static class StaticData
    {
        public static string[] ItemSamples =
        {
            @"Rarity: Rare
Grim March
Titan Greaves
--------
Quality: +20% (augmented)
Armour: 320 (augmented)
--------
Requirements:
Level: 68
Str: 148
Dex: 95
Int: 66
--------
Sockets: R-R-R-G 
--------
Item Level: 69
--------
12% increased Attack and Cast Speed if you've Killed Recently
--------
+57 to Armour
+75 to maximum Life
2.5 Life Regenerated per second
+39% to Fire Resistance
+39% to Lightning Resistance
25% increased Movement Speed
--------
Has Perandus Footprints Effect",
            @"Rarity: Rare
Loath Knuckle
Goliath Gauntlets
--------
Quality: +20% (augmented)
Armour: 181 (augmented)
--------
Requirements:
Level: 68
Str: 151
Dex: 123
Int: 115
--------
Sockets: G-R-B-R 
--------
Item Level: 74
--------
Attack with Decree of Light when you take a Critical Strike
--------
Adds 5-10 Physical Damage to Attacks
+44 to Dexterity
+74 to maximum Life
+44% to Cold Resistance
+23% to Fire Resistance
--------
Has Infernal Gloves",
            @"Rarity: Rare
Blood Knot
Two-Stone Ring
--------
Requirements:
Level: 38
--------
Item Level: 76
--------
+12% to Cold and Lightning Resistances
--------
Adds 9-15 Physical Damage to Attacks
22% increased Elemental Damage with Weapons
+18 to Intelligence
+76 to maximum Life
+35% to Fire Resistance
+21% to Lightning Resistance",
            @"Rarity: Rare
Oblivion Whorl
Amethyst Ring
--------
Requirements:
Level: 32
--------
Item Level: 69
--------
+11% to Chaos Resistance
--------
Adds 6-11 Physical Damage to Attacks
+173 to Accuracy Rating
28% increased Rarity of Items found
--------
Note: ~b/o 7 chaos",
            @"Rarity: Rare
Sol Scratch
Imperial Skean
--------
Dagger
Physical Damage: 17-69
Critical Strike Chance: 6.60%
Attacks per Second: 1.45
--------
Requirements:
Level: 66
Dex: 95
Int: 131
--------
Sockets: G 
--------
Item Level: 80
--------
40% increased Global Critical Strike Chance
--------
29% increased Spell Damage
+41 to Intelligence
Adds 49-78 Fire Damage to Spells
+38% to Global Critical Strike Multiplier
36% increased Critical Strike Chance for Spells
--------
Note: ~b/o 13 chaos
 ",
            @"Rarity: Rare
Sol Scratch
Imperial Skean
--------
One Handed Sword
Physical Damage: 17-69
Critical Strike Chance: 6.60%
Attacks per Second: 1.45
--------
Requirements:
Level: 66
Dex: 95
Int: 131
--------
Sockets: G 
--------
Item Level: 80
--------
40% increased Global Critical Strike Chance
--------
29% increased Spell Damage
+41 to Intelligence
Adds 49-78 Fire Damage to Spells
+38% to Global Critical Strike Multiplier
36% increased Critical Strike Chance for Spells
--------
Note: ~b/o 13 chaos
 ",
            @"Rarity: Rare
Sol Nock
Harbinger Bow
--------
Bow
Physical Damage: 35-91
Elemental Damage: 39-68 (augmented)
Critical Strike Chance: 7.05% (augmented)
Attacks per Second: 1.20
--------
Requirements:
Level: 68
Dex: 212
--------
Sockets: G 
--------
Item Level: 83
--------
41% increased Critical Strike Chance
--------
+22 to Dexterity
Adds 39-68 Cold Damage
+295 to Accuracy Rating",
            @"Rarity: Rare
Phoenix Call
Demon's Horn
--------
Wand
Physical Damage: 26-48
Critical Strike Chance: 8.00%
Attacks per Second: 1.20
--------
Requirements:
Level: 56
Int: 179 (unmet)
--------
Sockets: B B-B
--------
Item Level: 78
--------
18% increased Spell Damage
--------
+1 to Level of Socketed Fire Gems
20% increased Lightning Damage
Adds 26-49 Fire Damage to Spells
23% increased Stun Duration on Enemies
+19 to Accuracy Rating
5% increased Light Radius"
        };
    }
}